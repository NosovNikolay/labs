...
void print_dllist(dllist_t *list)
{
	...
	while (iter != NULL)
	{
		printf("{{\"%s\", \"%s\", \"%s\"} \"%s\", \"%s\", %3u\n",
			   iter->data.brand_1.country, iter->data.brand_1.name,
			   iter->data.appointment, iter->data.color,
			   iter->data.laptop_compartment ? "true" : "false", iter->data.volume);
		iter = iter->next;
	}
}
...
void print_two_words(dllist_t *list)
{
	...
	if (list == NULL) {
		printf("[ERROR/%s]: Pointer to the list can't be NULL\n",
		       __FUNCTION__);
		return;
	}
	...
	/* Пока элементы существуют */
	while (iter != NULL) {
		/* Печать элемента списка */
		if (has_two_words(&iter->data))
			printf("{{\"%s\", \"%s\", \"%s\"} \"%s\", \"%u\", %3u}\n",
			       iter->data.brand_1.name,
			       iter->data.brand_1.country,
			       iter->data.appointment, iter->data.color,
			       iter->data.laptop_compartment, iter->data.volume);
